...
public class SuperWeChatManager {
    ...
    public void saveAppContactList(List<User> contactAppList) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if (db.isOpen()) {
            db.delete(UserDao.TABLE_USER_NAME, null, null);
            for (User user : contactAppList) {
                ContentValues values = new ContentValues();
    ...
    public Map<String, User> getAppContactList() {
        ...
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select * from " + UserDao.TABLE_USER_NAME /* + " desc" */, null);
            while (cursor.moveToNext()) {
                String username = cursor.getString(cursor.getColumnIndex(UserDao.TABLE_COLUMN_NAME));
    ...
    synchronized public void deleteAppContact(String username){
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if (db.isOpen()){
            db.delete(UserDao.TABLE_USER_NAME,UserDao.TABLE_COLUMN_NAME + " = ?",new String[]{username});
        }
    }
}